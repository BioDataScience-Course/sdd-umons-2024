...
-   Appréhender des lois de distributions statistiques et leurs utilisations pratiques (distribution uniforme, binomiale, Poisson)
...
La réponse est 0.125, soit une fois sur huit observations, en moyenne, le temps d'attente enregistré sera compris entre 2 et 2.5 minutes.
...
Donc il faut observer pendant 1.33 min (1 min et 20 secs) pour avoir une chance sur trois d'observer l'arrivée d'un insecte butineur.
...
Le **coefficient binomial** $C^j_n$ vaut[^06-probabilites-correlation-2] : $$C^j_n = \frac{n!}{j!(n-j)!}$$

[^06-probabilites-correlation-2]: Le factoriel d'un nombre $n$, noté $n!$ est $1 \times 2 \times 3 \times ... \times n$, avec $0! = 1$.
...
Maintenant, nous pouvons poser la question autrement. Prenons un couple sain au hasard en Belgique, quelle est la probabilité que ce couple transmette la mucoviscidose à leur descendance ? Ne considérons pas ici les personnes elles-mêmes atteintes de la maladie qui prendront certainement des précautions particulières. Sachant qu'une personne sur 20 est porteuse du gène défectueux (hétérozygote) dans la population saine belge, la probabilité de former un couple doublement hétérozygote qui pourrait transmettre la maladie est de[^06-probabilites-correlation-3] :

[^06-probabilites-correlation-3]: Comme il y a la même proportion d'hommes et de femmes porteurs, nous avons 1/20 des hommes et 1/20 des femmes qui sont porteurs. Nous formons des couples au hasard en piochant un homme dans la population masculine et une femme dans la population féminine de manière indépendante. Dans ce cas, nous obtenons un couple double porteur à une fréquence de 1/20 \* 1/20 = 1/400.

```{r}
1/20 * 1/20
```

... soit un couple sur 400. Donc, globalement, les probabilités d'avoir des enfants sains sont beaucoup plus grandes que 0.75 si nous incluons tous les couples belges de parents sains porteurs ou non. Cette probabilité est de[^06-probabilites-correlation-4] :

[^06-probabilites-correlation-4]: Sur 400 couples, 399 ont une probabilité d'engendrer des enfants sains (porteurs hétérozygotes ou non porteurs confondus) de 100%. À cela il faut ajouter un couple sur 400 qui aura une probabilité de 75% de faire des enfants non atteints, car non homozygotes.
...
À nouveau, c'est les mêmes valeurs pour les quatre séries, aux arrondis près. Que donnent les coefficients de corrélation[^06-probabilites-correlation-5] ?

[^06-probabilites-correlation-5]: Ici, nous croisons les *x* et les *y*, et extrayons la diagonale de ce tableau qui correspond aux coefficients entre *x1* et *y1*, entre *x2* et *y2*, entre *x3* et *y3* et entre *x4* et *y4*, respectivement.
...
`r learnr("A06Lc_correlation", title = "Association de deux variables", toc = "Association de deux variables")`

```{r assign_A06Ia_correlation_I, echo=FALSE, results='asis'}
if (exists("assignment"))
  assignment("A06Ia_correlation", part = "I",
...
```{r assign_A06Ga_human_health_V, echo=FALSE, results='asis'}
...
Vous venez de terminer le module 6. Ce module vous a permis d'apprendre les bases des probabilités et les principales lois de distribution. Pour évaluer votre compréhension de cette matière, vous aviez les exercices suivants à réaliser :
