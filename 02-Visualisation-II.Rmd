...
Dans le module précédent, vous avez travaillé seul sur un projet avec **Git** et **Github**. Vous avez débuté par réaliser une copie locale de projet nommé **clone** Vous avez ensuite réalisé des état de sauvegarde nommé **commit** de vos fichiers au sein d'un dépôt. Pour terminer vous avez réaliser une synchronisation de votre dépôt local avec votre dépot distant (sur Github) via un pull suivi d'un push

Dans le module précédent, vous avez travaillé individuellement sur un projet en utilisant **Git** et **Github**. Vous avez commencé par créer une copie locale du projet. Cette action est un **clone**. Ensuite, vous avez effectué des sauvegardes de vos fichiers dans un dépôt, appelées **commit**. Pour finir, vous avez synchronisé votre dépôt local avec votre dépôt distant (sur Github) en effectuant un **pull** suivi d'un **push**.

Si GitHub et Git permettent de travailler seul sur ses dépôts, ils sont prévu pour collaborer. En tant que scientifique, vous serez amené à collaborer abondamment à l'avenir.
...
![](images/sdd1_02/github_issues1.png)
