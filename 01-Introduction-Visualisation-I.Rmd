...
-   Se familiariser avec les outils de base (SciViews Box, RStudio, logiciel R](<https://www.r-project.org>) [@R-base], GitHub, Git).
...
La section suivante est repliée. Il s'agit d'une section optionnelle néanmoins indispensable pour réaliser les exercices de ce cours qui vont être réalisés sur un ordinateur.

Si vous considérez que vous maîtrisez les éléments de base d'un ordinateur, le clavier, que vous êtes capables de trouver toute une série de caractères, dont des caractères spéciaux et des raccourcis clavier, vous avez la possibilité de sauter cette section repliée et de continuer votre lecture. En cas d'hésitation, dépliez la section afin d'approfondir vos connaissances.
...
L'objectif principal de ce module est de réaliser des graphiques en nuage de points. Il est de ce fait important de maitriser des éléments de géométrie (origine, abscisse, ordonnée, coordonnées d'un point dans un plan) mlais également des fonctions mathématiques comme les puissances et les logarithmes. Vous trouverez une nouvelles section replié juste après l'exercice ci-dessous pour vous rafraichir la mémoire concernant ces notions.
...
<summary>Notion élémentaire de géométrie et des fonctions mathématiques</summary>

[Éléments de base de la géométrie](https://mathix.org/cours_interactif/index.php?l=fr&n=3e&c=Rep%C3%A9rage) suivant sont indispensable afin de réaliser le module suivant : origine, abscisse, ordonnée, coordonnées d'un point dans un plan...
...
L'information est partout et en quantité inimaginable. Pour ne citer qu'une valeur, en 2020, la quantité d'information ajoutée chaque semaine sur Internet était de 1000 milliards de milliards. Afin de traiter même une toute petite partie de ces données en lien avec la biologie, vous devrez être un scientifique des données capable d'employer toute une série d'outils (logiciels).

À la fin de ce premier module, vous aurez réalisé votre première analyse complète en biologie, mais vous la réaliserez avec des outils professionnels que vous utiliserez durant toutes vos études et même après, dans votre profession en lien avec la biologie.
...
Le bouton RStudio vous conduit vers une page qui vous permet d'accéder à votre machine virtuelle hébergé sur Saturn Cloud dans l'organisation EcoNum. Vous observez qu'il n'est pas inscrit SciViews Box mais RStudio sur le bouton. Il s'agit du logiciel qui va nous permettre de réaliser vos rapports d'analyse.

![](images/sdd1_01/saturn_launch.png)

Cette page contient également toutes les instructions indispensables pour : - installer et configurer la SciViews Box, composée entre autres des logiciels R et RStudio nécessaires pour ce cours ; - allumer, éteindre, valider votre configuration.
...
Passons maintenant à la pratique. Lisez la suite de ce module. Nous réaliserons le projet sur le nuage de points ensemble en présentiel.

### GitHub Classroom {#classroom}

[**GitHub Classroom**](https://classroom.github.com) est une extension de GitHub. Rappelez-vous, l'une des premières étapes que vous avez réalisées dans le cadre de ce cours a été de créer votre compte GitHub. Sur GitHub, vous allez pouvoir sauvegarder sur le cloud, partager et collaborer sur vos travaux.

GitHub Classroom facilite le travail dans le contexte d'exercices de niveau III ou de niveau IV (les projets individuels et en groupes) à réaliser dans le cadre d'un cours. Vous serez amené à modifier des projets issus de GitHub Classroom pour réaliser vos exercices.

**Vos projets seront privés**. Seuls vous-mêmes et vos enseignants aurez accès à ces projets, mais vous pourrez aussi les rendre publics, si vous voulez *valoriser votre travail* de manière plus large.

L'illustration ci-dessous vous montre la boîte d'information qui vous permet d'initier votre projet. Cette boîte se trouve à la fin du module 1. Soyez vigilant quant à la date limite pour terminer le projet.

![](images/sdd1_01/gh_classroom.png)

Attention, si vous n’êtes pas reconnu, vous ne pourrez pas initier votre projet et vous serez renvoyé vers une page comme celle ci-dessous. Vous pourrez accéder à votre projet en vérifiant votre identification.
...
Démarrez votre SciViews box et affichez la fenêtre de RStudio maintenant (si vous ne l'avez pas encore installée, cliquez sur le bouton bleu **RStudio** en haut à droite de cette page pour voir les explications d'installation). RStudio permet de cloner (faire un copie locale sur votre ordinateur) directement un projet disponible sur GitHub en quelques étapes comme vous pouvez le voir ci-dessous.
...
Nous nous sommes intéressés à la partie collaborative de GitHub. Nous avons exploré la structures d'un projet et les outils de discussions que sont les issues. Nous allons maintenant nous intéresser à Git, le gestionnaire de version de projet utilisé par GitHub et par RStudio.
...
Un logiciel comme git vous offre une solution professionnelle pour gérer vos fichiers au sein d'un projet.

Le fichier urchin_notebook s'ouvre dans la zone d'édition. Le nom de l'auteur y est proposé. Une fois le document sauvegardé, on retrouve dans l'onglet Git le nom du fichier modifié. Le gestionnaire de version va suivre toutes les modifications apportées au fichier au sein du projet, qui est un dossier sur votre ordinateur. Ce dossier se nomme un dépôt (*repository* en anglais).

`r img("sdd1_01/git1.gif")`

Chaque bouton offre des fonctionnalités de git. Nous allons en découvrir une partie dans ce premier module.

![](images/sdd1_01/git_tab.png){width="50%"}

Les deux premiers boutons, `Diff` et `Commit`, ouvrent la même nouvelle fenêtre destinée à l'utilisation de Git.

![](images/sdd1_01/git_diff.png)

En bas de cette fenêtre, nous allons suivre les modifications apportées à nos documents. Dans notre exemple, il n'y en a qu'un seul. La liste des fichiers modifiés se trouve en haut à gauche de la fenêtre.

Dans le fichier urchin_notebook.qmd, on observe en rouge l'ancienne version et en vert la nouvelle version du document. Une modification a été réalisée à la troisième ligne du document. Comme vous pouvez vous en rendre compte, Git permet de suivre finement chaque modification d'un document.

Le logiciel Git permet de faire des points de sauvegarde de votre projet. Pour Git, il s'agit de réaliser un **commit**.

Pour ce faire, vous devez indexer les fichiers que vous souhaitez sauvegarder. Cette étape se réalise en sélectionnant le fichier urchin_notebook.qmd dans notre exemple. De plus, vous devez proposer un message de validation qui fournit une description brève des modifications apportées dans ce commit.

![](images/sdd1_01/git_commit.png)

Il ne reste plus qu'à cliquer sur le bouton commit afin de réaliser ce point de sauvegarde. Vous pouvez observer cette étape sur l'illustration ci-dessous. Des informations sont proposées, comme le nombre de fichiers modifiés.

`r img("sdd1_01/git_commit.gif")`

Notre dépôt de travail est également présent en ligne. Afin de synchroniser votre dépôt, nous vous conseillons de toujours commencer par rapatrier localement les changements réalisés. Cette étape se nomme un **pull**. Vous pouvez cliquer sur tirer (une flèche bleue vers le bas). Elle sera cruciale lors d'un travail à plusieurs. Sur l'illustration suivante, vous pouvez observer que vous êtes à jour (Already up to date).

`r img("sdd1_01/git_pull.gif")`

L'étape suivante consiste à envoyer vos changements locaux vers la version sur le "cloud". Cette étape se nomme un **push**. Il s'agit de la flèche verte vers le haut. Plusieurs **commits** peuvent être envoyés avec un seul **push** sur le réseau. Lisez le message afin de vous assurer que l'envoi a pu être réalisé.

`r img("sdd1_01/git_push.gif")`

En cliquant sur Historique, vous pouvez suivre la progression des commits au cours du temps. Sur l'illustration, on observe trois commits successifs.
...
Dans ce schéma, le flux est linéaire. Pour enregistrer une nouvelle version de votre document, vous effectuez un **commit** accompagné d'un message expliquant les modifications apportées.
...
Lorsqu'on rédige un document, on perd beaucoup de temps à décider de l'interligne, de choisir la taille et la police... On peut diviser la rédaction d'un document en deux grandes étapes : le fond et la forme. Nous vous proposons un outil pour booster votre **productivité** (et donc pour gagner du temps) avec Markdown. Dans un premier temps, nous allons nous intéresser uniquement au fond. Ensuite, nous déterminerons comment le document sera mis en forme.
...
```{block, type='note'}
Il vous est toujours possible d'exécuter ces instructions ligne après ligne dans la fenêtre **Console** pour les tester tout comme à partir d'un script R.
```

Markdown est relativement simple et intuitif à l'usage, même si un petit effort est nécessaire au début. Il permet de *baliser* le texte pour indiquer le sens des différentes parties (par exemple, pour indiquer les différents niveaux de titres). L'apparence finale du titre sera, quant à elle, définie dans une feuille de style séparée. Le Markdown est employé dans les documents Quarto/R Markdown et dans les issues sur GitHub. Par défaut, dans RStudio, un mode visuel est proposé, c'est-à-dire qu'on a la possibilité d'employer des boutons pour mettre en gras, en italique, etc. Bien que ce mode soit utile, nous vous recommandons de faire l'effort d'apprendre les balises du Markdown.
...
Dans le cadre de ce cours, vous allez vous poser de nombreuses questions. Ces questions pourront porter sur l'utilisation des logiciels, sur la théorie statistique derrière un concept ou encore sur l'utilisation d'une fonction particulière. Cette section va vous donner des pistes afin de trouver de l'aide par vous-mêmes ou en faisant appel à la communauté.

Il est indispensable d'apprendre à rechercher de l'information de qualité afin de répondre à ces questions.

#### Pages d'aide

Concernant les fonctions dans R, des pages d'aide sont associées à chacune d'elles. Pour les lire, utilisez l'opérateur `?` suivi du nom de la fonction. Dans RStudio, la page d'aide apparaît dans l'onglet **Aide**. Dans RStudio, il est encore bien plus facile de placer le curseur sur le nom de la fonction dans le code et d'appuyer sur la touche `F1` pour faire apparaître sa page d'aide.
...
Une page d'aide est en anglais et bien souvent technique. Dans certain cas, la page d'aide ne va pas être suffisante afin de vous permettre de répondre à votre interrogation.

#### Chatbot SciViews

Le chatbot SciViews est un grand modèle de langage, plus connu sous sa dénomination anglaise de LLM ou encore de *large language model*. Derrière ChatGPT, Perplexity, Copilot et Gemini se cachent des LLM. Le chatbot est disponible via les addins dans RStudio afin de répondre à vos questions. Il vous est mis à disposition dans le cadre de ce cours, qui traite du langage R, des statistiques et de la science des données.

<!-- Ajouter une explication sur l'addins -->

Ces réponses apparaîtront comme une page d'aide dans RStudio.
...
Néanmoins, si vous ne trouvez pas la réponse à votre question malgré une recherhce dans les apge d'aide de R, l'utilisation du chatbot et une bonne recherche sur l'Internet. Il vous reste la possibilité de faire appel à la communauté.

Pour tous les étudiants du cours de Sciences des données I, un espace d'échange est disponible pour poser des questions entre les membres du cours de Science des données I, vous pouvez y accéder en utilisant l'addins depuis RStudio. En trois étape, vous posez simplement votre question.

1.  Depuis RStudio, je clique sur l'addin qui se trouve dans le menu déroulant `Addins`
...
Vous pouvez également accéder à cet espace de discussion depuis le site du cours.
